...
// Hash function
func (m *OAuthClientCredentials) Hash(hasher hash.Hash64) (uint64, error) {
	if m == nil {
		return 0, nil
	}
	if hasher == nil {
	...
	var err error
	if _, err = hasher.Write([]byte("enterprise.gloo.solo.io.github.com/solo-io/gloo/projects/gloo/pkg/api/v1/enterprise/options/extauth/v1.OAuthClientCredentials")); err != nil {
		return 0, err
	}

	if _, err = hasher.Write([]byte(m.GetAuthUrl())); err != nil {
		return 0, err
	}
...
func (m *AuthConfig_Config) Hash(hasher hash.Hash64) (uint64, error) {
	...
	case *AuthConfig_Config_OauthCc:

		if h, ok := interface{}(m.GetOauthCc()).(safe_hasher.SafeHasher); ok {
			if _, err = h.Hash(hasher); err != nil {
				return 0, err
			}
		} else {
			if val, err := hashstructure.Hash(m.GetOauthCc(), nil); err != nil {
				return 0, err
			} else {
				if err := binary.Write(hasher, binary.LittleEndian, val); err != nil {
					return 0, err
...
func (m *ExtAuthConfig_Config) Hash(hasher hash.Hash64) (uint64, error) {
	...
	case *ExtAuthConfig_Config_OauthCc:
